...
trait ApiUserRequestRecordComponent extends BaseComponent with ApiUserTableDefinition { this: DatabaseConfig =>
  ...
  class ApiUserRequestRecordTable(tag: Tag) extends BaseTable[ApiUserRequestRecord](tag, "api_user_request_record") {
    ...
    val requestInput = column[String]("request_input", O.Length(256, varying = true))

    val apiUsersKey =
      foreignKey("API_USER_ID_FK", userId, apiUsersTable)(_.id, onDelete = ForeignKeyAction.Cascade)

    //Add id to *
    def * = (id, userId, requestInput, createdAt) <> (ApiUserRequestRecord.tupled, ApiUserRequestRecord.unapply)
